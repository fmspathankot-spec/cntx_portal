...
/**
 * Get SFP info for interface
 * @param {Object} router - Router object
 * @param {string} interfaceName - Interface name (e.g., '1/1/1')
 * @returns {Promise<string>} SFP info output
 */
async function getSFPInfo(router, interfaceName) {
  return await executeCommand(router, `show interface ${interfaceName} transceiver`);
}

/**
 * Get SFP stats for interface
 * @param {Object} router - Router object
 * @param {string} interfaceName - Interface name
 * @returns {Promise<string>} SFP stats output
 */
async function getSFPStats(router, interfaceName) {
  return await executeCommand(router, `show interface ${interfaceName} optics`);
}
