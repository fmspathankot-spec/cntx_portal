...
export async function GET(request) {
  try {
    ...
    console.log(`[BGP-PROXY] Forwarding request to Python backend for router ID: ${routerId}`);
    
    // Call Python backend
    const pythonUrl = `${PYTHON_BACKEND_URL}/api/tejas/live/bgp?routerId=${routerId}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[BGP-PROXY] Python backend error:`, errorData);
        
        return NextResponse.json(
          { 
            success: false, 
            error: 'Python backend error',
            message: errorData.error || errorData.message || 'Failed to fetch BGP data',
      ...
      console.log(`[BGP-PROXY] Success: Received data from Python backend`);
      
      return NextResponse.json(data);
      
    } catch (fetchError) {
      console.error('[BGP-PROXY] Error calling Python backend:', fetchError.message);
    ...
    console.error('[BGP-PROXY] Error:', error);
    return NextResponse.json(
      { 
        success: false, 
