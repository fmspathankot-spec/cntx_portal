"""
Tejas Router Monitoring Script
Monitors OSPF, BGP, SFP 100G Info and Stats
Based on actual Tejas command outputs
"""
...
class TejasCommandParser:
    ...
    def parse_ospf_neighbors(output):
        ...
        for line in lines:
            ...
            if in_table and line.strip():
                # Parse neighbor line
                # Format: Neighbor-ID  Pri  State  DeadTime  Address  Interface  Helper  HelperAge  HelperER  Bfd  AreaID
                parts = line.split()
    ...
    def parse_sfp_100g_stats(output):
        ...
        # Extract RX power for all lanes
        ...
        # Extract TX power for all lanes
        ...
        # Extract bias current for all lanes
...
class DatabaseManager:
    ...
    def get_tejas_routers(self):
        ...
        try:
            ...
            query = """
                SELECT id, hostname, ip_address as host, ssh_port as port,
                       username, password, device_type
                FROM routers
                WHERE is_active = true AND device_type = 'tejas'
                ORDER BY hostname
            """
...
class TejasRouterMonitor:
    ...
    def monitor_router(router, interfaces, db_manager):
        ...
        try:
            ...
            # 3. Monitor SFP for each interface
            for interface in interfaces:
                ...
                # SFP Info
                logger.info(f"    🔍 Getting SFP info...")
                ...
                # SFP Stats
                logger.info(f"    🔍 Getting SFP stats...")
...
def display_results(all_results):
    ...
    for router_name, results in all_results.items():
        ...
        if results['bgp']:
            ...
            print(f"    Forwarding: {results['bgp'].get('forwarding_state', 'N/A')}")
        ...
        if results['interfaces']:
            ...
            for interface_name, interface_data in results['interfaces'].items():
                ...
                if interface_data['sfp_info']:
                    ...
                    print(f"      Status: {info.get('operational_status', 'N/A')}")
                    print(f"      Laser: {info.get('laser_type', 'N/A')}")
...
def main():
    """Main execution"""
    logger.info("🚀 Starting Tejas Router Monitoring")
